...
**安装过程**：
1. 检查系统环境
2. 列出可用网卡，选择 WAN（外网）和 LAN（内网）接口
3. 检查网络连接
4. 输入管理员密码（用于登录管理后台）
...
4. 直接访问 http://192.168.100.1/ 选择代理节点

---

## 常见问题

### Q1: 如何区分哪个网卡是外网，哪个是内网？

A: 安装脚本会列出所有可用网卡并显示连接状态，你只需根据提示选择即可。如果需要手动确认：
1. 只插外网网线，运行 `ip addr show`，有 IP 的是外网网卡
2. 再插内网网线，剩下的就是内网网卡

### Q2: 安装时看不到可用的网卡？

A: 安装脚本会列出所有可用网卡供你选择 WAN 和 LAN 接口。如果列表为空或缺少网卡：
1. 网线是否插好（有线网卡需要连接才能检测到）
2. 运行 `ip link show` 查看网卡列表
3. USB 网卡运行 `lsusb` 查看 USB 设备
4. 尝试更换 USB 接口

### Q3: 客户端无法获取 IP？

A: 检查：
1. 网线是否连接到 LAN 网卡（安装时选择的内网接口）
2. LAN 网卡是否配置为 192.168.100.1
...
A: 检查：
1. WAN 网卡是否有 IP：`ip addr show`（查看安装时选择的外网接口）
