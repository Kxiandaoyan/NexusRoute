...
当你新增一台设备时，系统会自动完成所有配置，无需任何人工干预：

```
新设备连接到网关内网接口
    ↓
dnsmasq DHCP 自动分配永久 IP (192.168.100.10-209)，无限租期
    ↓
Node.js 后端自动发现新设备（15秒内）
    ↓
后端自动执行以下操作：
  ...
  ✅ 添加该用户的 iptables TPROXY 规则
    ↓
设备自动获得永久 IP，可以访问 http://192.168.100.1/
    ↓
用户在网页上选择代理节点（1-3跳）
    ↓
...
**其他所有配置都是自动的！** 新设备即插即用，无需手动审批、配置 IP、创建配置文件或编写防火墙规则。
...
安装脚本会自动检测可用网卡，你只需根据提示选择哪个是 WAN、哪个是 LAN。
...
> **提示**：安装脚本支持重复执行。如果配置有误，直接重新运行 `sudo ./install.sh` 即可。
...
1. 将你的电脑/手机连接到网关的内网接口（LAN 网卡）
2. 设备会自动获取永久 IP（192.168.100.10-209）
3. 系统在15秒内自动完成所有配置（用户、iptables、Xray）
4. 无需任何手动操作，即插即用
...
**答案**：不会！NexusRoute 会立即切断网络。同时，系统会**完全禁用 IPv6**，杜绝 IPv6 泄露风险。
...
### Q2: 支持多少个设备？
A: 最多 200 个设备（DHCP 分配范围 192.168.100.10-209）。
...
### v3.0 (2026-04-10)
- 🎉 移除设备审批流程，新设备即插即用，全自动注册
- 📡 DHCP 改为永久租期，单一 IP 范围（10-209），设备接入即获永久 IP
- 🔧 安装脚本支持动态选择 WAN/LAN 网卡，不再硬编码 eth0/eth1
- 🔄 安装脚本支持重复执行，配置有误可一键重装
- 🛡️ IPv6 完全禁用，作为 Kill Switch 的一部分防止泄露
- 🗑️ 移除升级脚本（update.sh），直接重新运行 install.sh 即可
- 📚 管理后台精简为：节点管理、用户列表、系统管理
